...
def load_model(filename="Two_Wh_RangePred_RFR_Model.pkl"):
    """Loads the saved CatBoostRegressor model from a file."""
    try:
        model = RandomForestRegressor(n_estimators=809,
                                     min_samples_split=2,
                                     min_samples_leaf=4,
                                     max_depth=11)
...
def predict_energy_consumption(model, data, selected_max_speed):
    ...
    X = X[['b', 'crr', 'Peak Motor Power Kw', 'drr', 'Final Gear',
           'Independent_GVW', 'Running Cycle_IDC', 'Running Cycle_WMTC',
           'Max_Speed_25', 'Max_Speed_30', 'Max_Speed_35', 'Max_Speed_40',
           'Max_Speed_45']]
    ...
    # X_array = X.to_numpy()
    # y_pred = round(model.predict(X_array)[0],1)
    y_pred = round(model.predict(X)[0],1)
